...
**In countries that didnt have revolution the growth rate was higher based on the mean. In countries where no revolutions happened their real gdp in 1960 was higher**: type one or two sentences describing this table of a fellow student below. For example, comment on the mean and median growth of both groups. Then stage, commit and push it to their github repository. 
Based 
...
```{r}
list(model1, model2, model3, model4) |>
  modelsummary(stars=T, gof_map = c("nobs", "r.squared"), title = "Regression Table", output = 'table_1.docx')
```
